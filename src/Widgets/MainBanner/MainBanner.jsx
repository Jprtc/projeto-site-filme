...
function MainBanner({ idioma }) {
  ...
  const textos = {
    pt: {
      ...
      chamadaEmail:
        "Quer assistir? Informe seu e-mail para criar ou reiniciar sua assinatura.",
      placeholderEmail: "Email",
      botaoVamosLa: "Vamos lá",
      mensagemSucesso:
        "E-mail {email} registrado! Redirecionando para o plano...",
    },
    en: {
      titulo: "Unlimited movies, TV shows, and more",
      subtitulo: "Starts at $4.03. Cancel anytime.",
      chamadaEmail:
    ...
    es: {
      ...
      subtitulo: "Desde 3,54 €. Cancela cuando quieras.",
      chamadaEmail:
    ...
    fr: {
      ...
      subtitulo: "À partir de 3,54 €. Annulez à tout moment.",
      chamadaEmail:
        "Prêt à regarder ? Saisissez votre adresse e-mail pour créer ou redémarrer votre abonnement.",
      placeholderEmail: "Adresse e-mail",
      botaoVamosLa: "Commencer",
      mensagemSucesso:
        "E-mail {email} enregistré ! Redirection vers le plan...",
    },
    de: {
      titulo: "Unbegrenzt Filme, Serien und mehr",
      subtitulo: "Ab 3,54 €. Jederzeit kündbar.",
      chamadaEmail:
        "Bereit zum Streamen? Gib deine E-Mail-Adresse ein, um deine Mitgliedschaft zu erstellen oder neu zu starten.",
    ...
    it: {
      ...
      subtitulo: "A partire da 3,54 €. Disdici quando vuoi.",
      chamadaEmail:
        "Vuoi guardare Netflix? Inserisci la tua email per creare o riattivare il tuo abbonamento.",
    ...
    ja: {
      titulo: "映画、ドラマなどが見放題",
      subtitulo: "月額¥660から。いつでもキャンセル可能。",
      chamadaEmail:
        "視聴を開始しますか？メールアドレスを入力して、メンバーシップを開始または再開してください。",
    ...
    ko: {
      ...
      subtitulo: "월 ₩5,518부터 시작. 언제든지 취소 가능.",
      chamadaEmail:
  ...
  // Fallback: se "idioma" vier undefined ou com um valor que não existe no objeto acima,
  // usamos "pt" como padrão em vez de quebrar a aplicação.
  const dados = textos[idioma] || textos.pt;

  const handleSubmit = (e) => {
    e.preventDefault();

    const msg = dados.mensagemSucesso.replace("{email}", emailDigitado);
    alert(msg);
  };
  ...
  return (
    <div className="banner-principal-container">
      ...
      <div className="banner-conteudo">
        <h1 className="banner-titulo">{dados.titulo}</h1>
        <p className="banner-subtitulo">{dados.subtitulo}</p>
        <p className="banner-chamada-email">{dados.chamadaEmail}</p>

        <form className="banner-formulario" onSubmit={handleSubmit}>
          <input
            type="email"
            placeholder={dados.placeholderEmail}
